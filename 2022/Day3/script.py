...
vJrwpWtwJgWr, while the second compartment contains the items hcsFMMfFFhFp. The only item type that appears in both compartments
is lowercase p.
...
Every set of three lines in your list corresponds to a single group, but each group can have a different badge item type. So,
...
class group:
    ...
    def find_key_item(self) -> str:
        iteration_records = self.records
        result = set([*self.records[0].string])
        iteration_records.pop(0)

        for record in iteration_records:
            query = set([*record.string])
            result.intersection_update(query)
...
class sack:
    def __init__(self, input: str) -> None:
        self.string1, self.string2 = line[:len(input)//2], line[len(input)//2:]
        self.string = input
        pass
...
with open('2022/Day3/test.txt') as f:
    for line in f:
        record = sack(line.strip())

        if len(records) < 3:
            records.append(record)

        if len(records) == 3:
            new_group = group(records)
            groups.append(new_group)
            total = total + get_priority(new_group.key)
            records.clear()
